...
async def predict_batch_file(file: UploadFile = File(...)):
    try:
        ...
        # Dividir en 80% entrenamiento y 20% prueba con split estratificado
        X_train, X_test, y_train, y_test = train_test_split(
        ...
        if len(class_distribution) < 2:
            return {
                "status": "error",
                "message": "El conjunto de entrenamiento no contiene ambas clases.",
                "details": f"Distribución de clases en entrenamiento: {class_distribution.to_dict()}",
            }
